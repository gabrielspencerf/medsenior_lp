...
export const Hero: React.FC<HeroProps> = ({ locationData, onCtaClick }) => {
  return (
    <section id="home" className="relative bg-white min-h-0 flex items-center pt-6 pb-8 lg:pt-10 lg:pb-12 overflow-hidden">
      {/* Navigation anchors */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">

        {/* Content – no mobile tudo centralizado; no desktop alinhado à esquerda */}
        <div className="space-y-2 lg:space-y-4 text-center lg:text-left order-2 lg:order-1">
          <div className="flex flex-col items-center gap-1 sm:flex-row sm:items-center sm:gap-2 lg:justify-start">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand-light border border-brand-primary/20 w-fit">
              <span className="flex h-2 w-2 rounded-full bg-brand-primary animate-pulse"></span>
              <span className="text-brand-dark text-sm font-bold tracking-wide uppercase">Canal de Cotação {locationData.name}</span>
            </div>
            <span className="text-sm text-gray-600 font-medium">
              Atendimento por consultor de vendas.
            </span>
          </div>
          ...
          <p className="text-lg text-gray-600 max-w-lg mx-auto lg:mx-0 leading-relaxed" dangerouslySetInnerHTML={{
            __html: locationData.hero.description || `Aproveite a tabela exclusiva para <strong>novas contratações</strong> em ${locationData.name}. Hospital Próprio, Pronto-Atendimento 24h e análise de carência conforme regras do plano.`
          }} />

          {/* Explicit Sales Disclaimer Box */}
          <div className="bg-red-50 border-l-4 border-red-400 p-3 text-left text-base text-gray-700 rounded-r-lg max-w-lg mx-auto lg:mx-0 shadow-sm">
            <div className="flex items-start gap-3">
              <Info className="w-5 h-5 text-red-500 mt-0.5 flex-shrink-0" aria-hidden />
              <div>
                <p className="font-bold text-red-800 uppercase text-sm mb-1">Aviso Importante</p>
                <p className="leading-relaxed">Este site é um canal para <strong>quem deseja contratar o plano</strong>. Se você já é cliente, ligue para o atendimento no verso da sua carteirinha.</p>
              </div>
            </div>
          </div>
          ...
          <p className="text-base text-gray-600 pt-1">
            Cotação para você ou para um familiar.
          </p>

          <div className="flex items-center justify-center lg:justify-start gap-4 sm:gap-6 pt-2 lg:pt-4 text-base text-gray-600 font-medium border-t border-gray-100 mt-2 lg:mt-4 w-full lg:w-max">
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-5 h-5 text-brand-primary shrink-0" aria-hidden />
              <span>Contratação Segura</span>
            </div>
            <div className="flex items-center gap-2">
              <Star className="w-5 h-5 text-brand-primary shrink-0" aria-hidden />
        ...
        <div className="relative order-1 lg:order-2 flex flex-col items-center justify-center mt-0 lg:mt-0">
          <div className="relative w-full max-w-md lg:max-w-2xl">
            ...
            <div className="relative w-full aspect-video rounded-xl overflow-hidden shadow-2xl border-4 border-white bg-gray-100 z-10 select-none">
              <img
            ...
            {locationData.hero.differential && (
              <div className="mt-3 bg-white p-3 rounded-xl shadow-lg border border-gray-100 w-full flex items-center gap-4 relative z-10">
                <div className="bg-brand-light p-3 rounded-full text-brand-primary flex-shrink-0">
                  <Clock size={24} />
                </div>
                <div>
                  <p className="text-sm font-bold text-gray-600 uppercase mb-0.5">{locationData.hero.differential.title}</p>
